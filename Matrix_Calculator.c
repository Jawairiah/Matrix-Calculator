	...
	while(1)
	{
    	...
    	if (option==1)
		{
    	    ...
    	    if (choice == 7 || choice == 9)
    	    {
    	    	if (rA>3 && cA>3)
    	    	{
    	    		printf(COLOR_RED"\nCalculator cannot solve for this matrix size.\n"COLOR_RESET);
    	    		sleep(2);
					printf("\n");
    	    		system ("cls");
    	    		continue;
        	...
        	else if(choice==9)
        	{
    		    ...
    		    if(rA==1 && cA==1)
                {
                	...
                	for(i=0; i<rA; i++)
                	{
            	    	for(j=0; j<cA; j++)
            		    {
            		    	printf(COLOR_YELLOW" %d "COLOR_RESET,inv[i][j]);
            	    	}
	                	printf("\n");
                	}  
            	}
